...
float interpol_value = 80.0; // Increased by more than 2x (original was 15.0) for more interpolated lines
  ...
  for (uint i=0; i< ZI.n_rows - interpol_value; i+=1)
   {       
      for (uint j=0; j<ZI.n_cols ; j+=1)
      {
        ...
        ang = fmod(ang + 2.0 * M_PI, 2.0 * M_PI); // Normalize angle for overflow
