...
#include <omp.h>
#else
// Define empty macros if OpenMP is not available
...
#pragma omp parallel for collapse(2)
#endif
    for (arma::uword i = 2; i < Z.n_rows - 2; ++i) {
        for (arma::uword j = 2; j < Z.n_cols - 2; ++j) {
            if (Z(i, j) == 0) {  // Missing data point
                ...
                for (int di = -base_window; di <= base_window; ++di) {
                    for (int dj = -base_window; dj <= base_window; ++dj) {
                        arma::sword ni = static_cast<arma::sword>(i) + di;
                        arma::sword nj = static_cast<arma::sword>(j) + dj;
                        if (ni >= 0 && nj >= 0 && 
                            static_cast<arma::uword>(ni) < Z.n_rows && 
                            static_cast<arma::uword>(nj) < Z.n_cols && 
                            Z(static_cast<arma::uword>(ni), static_cast<arma::uword>(nj)) > 0) {
                            local_avg_distance += Z(static_cast<arma::uword>(ni), static_cast<arma::uword>(nj));
                            valid_count++;
                        }
                    }
                }
                    ...
                    for (int di = -adaptive_window; di <= adaptive_window; ++di) {
                        for (int dj = -adaptive_window; dj <= adaptive_window; ++dj) {
                            ...
                            if (ni >= 0 && nj >= 0 && 
                                ...
                                weighted_sum += Z(static_cast<arma::uword>(ni), static_cast<arma::uword>(nj)) * weight;
                                weight_total += weight;
                            }
                        }
                    }
...
PointCloud::Ptr generateDensePointCloud(const arma::mat& ZI, const arma::mat& ZzI, 
                                        float maxlen_param, float min_FOV_param, float max_FOV_param) {
...
#pragma omp parallel for collapse(2)
    ...
    for (arma::uword i = 0; i < ZI.n_rows; i++) {
        for (arma::uword j = 0; j < ZI.n_cols; j++) {
            if (ZI(i, j) > minlen && ZI(i, j) < maxlen_param) {
                
                float ang = M_PI - ((2.0 * M_PI * j) / (ZI.n_cols));
                
                if (ang >= min_FOV_param - M_PI/2.0 && ang <= max_FOV_param - M_PI/2.0) {
                    ...
                    point.y = point_vec(1);
                    point.z = point_vec(2);
                    point.intensity = pc_modulo / maxlen_param * 255.0;  // Normalized intensity
...
#pragma omp critical
#endif
                    {
                        ...
                        if (pc_modulo > maxlen_param * 0.3) {  // Only for medium to far ranges
...
arma::mat optimizedNoiseReduction(const arma::mat& ZI, double max_var_param, float maxlen_param, float interpol_value_param) {
    arma::mat result = ZI;
    
#ifdef _OPENMP
#pragma omp parallel for
#endif
    for (arma::uword i = 0; i < ((ZI.n_rows-1) / static_cast<arma::uword>(interpol_value_param)); i++) {
        for (arma::uword j = 0; j < ZI.n_cols - 5; j++) {
            ...
            for (arma::uword k = 0; k < static_cast<arma::uword>(interpol_value_param); k++) {
                arma::uword row_idx = (i * static_cast<arma::uword>(interpol_value_param)) + k;
            ...
            if (valid_points > 2) {  // Need at least 3 points for meaningful variance
                ...
                double adaptive_threshold = max_var_param;
                if (avg_distance > maxlen_param * 0.6) {
                    adaptive_threshold = max_var_param * 1.5;  // More lenient for far regions
                }
                
                if (variance > adaptive_threshold) {
                    for (arma::uword m = 0; m < static_cast<arma::uword>(interpol_value_param); m++) {
                        arma::uword row_idx = (i * static_cast<arma::uword>(interpol_value_param)) + m;
...
#pragma omp parallel for
#endif
    for (int i = 0; i < static_cast<int>(dense_cloud->points.size()); i++) {
        ...
        if (px_data >= 0 && px_data < static_cast<int>(cols) && 
            py_data >= 0 && py_data < static_cast<int>(rows)) {
            ...
            point.r = static_cast<int>(color[2]); 
            point.g = static_cast<int>(color[1]); 
            point.b = static_cast<int>(color[0]);
...
#pragma omp critical
            ...
            int color_dis_x = static_cast<int>(255 * (distance / maxlen));
...
int main(int argc, char** argv) {
    ...
    // Set OpenMP threads for optimal performance (only if OpenMP is available)
#ifdef _OPENMP
    int max_threads = std::min(8, static_cast<int>(std::thread::hardware_concurrency()));
    omp_set_num_threads(max_threads);
    ROS_INFO("OpenMP enabled with %d threads", max_threads);
#else
    ROS_INFO("OpenMP not available, running single-threaded");
#endif
