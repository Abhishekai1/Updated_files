...
float angular_resolution_y = 1.0f;
...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2>& in_pc2 , const ImageConstPtr& in_image)
{
  ...
  for (uint i = 0; i < ZI.n_rows; i++) {
      for (uint j = 0; j < ZI.n_cols; j++) {
          if (ZI(i, j) == 0) {
              if (i + interpol_value/3 < ZI.n_rows) {
                  for (int k = 1; k <= interpol_value/3; k++) {
                      Zout(i + k, j) = 0;
                  }
              }
              if (i > interpol_value/3) {
                  for (int k = 1; k <= interpol_value/3; k++) {
                      Zout(i - k, j) = 0;
                  }
              }
  ...
  double density_threshold = 0.03;  // Lowered for far-range sparsity
  int max_window_size = 15;  // Increased for better far-range gap-filling
  ...
  // Set edge threshold
  double edge_threshold = 0.05 * arma::max(arma::max(grad_mag));  // Lowered for far-range edges
  ...
  // Replace ZI with edge-preserved Zenhanced
  ZI = Zenhanced;
  ...
  if (f_pc) {    
    ...
    for (uint i=0; i< ((ZI.n_rows-1)/interpol_value); i+=1)       
        for (uint j=0; j<ZI.n_cols-5 ; j+=1)
        {
          ...
          if (promedio > maxlen * 0.5) varianza *= 1.5; // Lenient for far ranges
   ...
   {       
      for (uint j=0; j<ZI.n_cols ; j+=1)
      {
        ...
        {  
          ...
          Eigen::MatrixXf Lidar_matrix(3,3); //matrix transformation between lidar and range image
  ...
  for (int i = 0; i < size_inter_Lidar; i++)
  {
      ...
      float distance = sqrt(P_out->points[i].x * P_out->points[i].x + P_out->points[i].y * P_out->points[i].y);
      int color_dis_x = (int)(255*((distance/maxlen)*1.2)); // Boost for far-range visibility
